...
*  =======
*
*  PSSCAL  multiplies  an  n  element  subvector  sub( X ) by the scalar
*  alpha,
*
*  where
*
...
*/
   ...
   if( !( info = ( ( nprow == -1 ) ? -( 601 + CTXT_ ) : 0 ) ) )
      PB_Cchkvec( ctxt, "PSSCAL", "X", *N, 1, Xi, Xj, Xd, *INCX, 6, &info );
   if( info ) { PB_Cabort( ctxt, "PSSCAL", info ); return; }
#endif
/*
